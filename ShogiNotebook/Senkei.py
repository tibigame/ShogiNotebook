...
from ShogiNotebook.Board import MoveDetail
from ShogiNotebook.util import xor
from ShogiNotebook.util import reverse_bw
from ShogiNotebook.util import d_print
...
class KingRookTrace(SenkeiPartsBase):
    ...
    def __move_black_p(self, m_d: MoveDetail):
        if m_d.pos == (2, 7):  # 26歩
            self.b_rook_front = 1
    ...
    def __move_white_p(self, m_d: MoveDetail):
        if m_d.pos == (8, 3):  # 84歩
            self.w_rook_front = 1
    ...
    def __move_black_r(self, m_d: MoveDetail):
    ...
    def __move_white_r(self, m_d: MoveDetail):
    ...
    def __move_black_k(self, m_d: MoveDetail):
        self.b_king.append(m_d.moved)

    def __move_white_k(self, m_d: MoveDetail):
...
class Yokofu:  # 横歩クラス
    def __init__(self, edge_p36: EdgeP36, rook_trace: KingRookTrace,
                 black_king_left_castle: KingLeftCastle, white_king_left_castle: KingLeftCastle,
                 tesuu: int, debug=False):
        self.debug = debug
        self.edge_p36 = edge_p36
        self.rook_trace = rook_trace
        self.black_king_left_castle = black_king_left_castle
        self.white_king_left_castle = white_king_left_castle
        self.form = None

    def __decide_form(self):
        p_str = self.edge_p36.form_str()
        p_tezon = self.edge_p36.tezon()
        if self.rook_trace.b_rook[-1] == (3, 4) and self.rook_trace.w_rook[-1] != (7, 6):  # 先手が横歩を取った
            get_side_p = "先手横歩取り"
        elif self.rook_trace.w_rook[-1] == (7, 6) and self.rook_trace.b_rook[-1] != (3, 4):  # 後手が横歩を取った
            get_side_p = "後手横歩取り"
        else:
            raise ValueError("どちらも横歩を取っていません")
        tesuu = 15
        gosenzo = self.__gosenzo_check()
        if gosenzo == "b":
            tesuu = 13
        elif gosenzo == "w":
            tesuu = 14
        elif get_side_p == "後手横歩取り":
            tesuu = 16
        return get_side_p

    def __gosenzo_check(self):
        if len(self.rook_trace.b_rook) == 1:
            return "b"
        if len(self.rook_trace.w_rook) == 1:
            return "w"
        return "n"
...
class Senkei:
    ...
    def __komadai(self, m_d: MoveDetail):
    ...
    def move(self, m_d: MoveDetail, tesuu=1):
        ...
        if self.senkei_classify.phylum == "横歩取り":  # 横歩取りの専用ルーチンを書く
            ...
            self.yokofu = Yokofu(self.edge_p36, self.rook_trace, self.black_king_left_castle, self.white_king_left_castle, tesuu)
